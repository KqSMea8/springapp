...
@IonicPage()
@Component({
  selector: 'page-school-certificates',
  templateUrl: 'school-certificates.html',
})
export class SchoolCertificatesPage {
  file: File;
  i: number = 0;
  f: any[] = [];
  employeeCode: any;
  ...
  uploadImage() {
    ...
    form.append('ed_emp_id', this.employeeCode)
    ...
    this._http.postRequest('http://localhost:8080/uploadedudocuments/upload-edu-document', form).subscribe(data => {
      console.log(data);
    });
